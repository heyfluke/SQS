...
DEFAULT_HOST = 'sqs.amazonaws.com'
PORTS_BY_SECURITY = { True: 443, False: 80 }
METADATA_PREFIX = 'x-amz-meta-'
AMAZON_HEADER_PREFIX = 'x-amz-'

# generates the aws canonical string for the given parameters
def canonical_string(method, accountid="", queue="", query_args={}, headers={}, expires=None):
    interesting_headers = {}
    ...
    # append the accountid if it exists
    if accountid != "":
        buf += "/%s" % accountid

    # add the queue.  even if it doesn't exist, add the slash
    buf += "/%s" % urllib.quote_plus(queue)
...
class CallingFormat:
    ...
    def build_url_base(protocol, server, port, accountid, calling_format):
        url_base = '%s://' % protocol

        if accountid == '':
            url_base += server
        elif calling_format == CallingFormat.SUBDOMAIN:
            url_base += "%s.%s" % (accountid, server)
        ...
        if (accountid != '') and (calling_format == CallingFormat.PATH):
            url_base += "/%s" % accountid
...
class AWSAuthConnection:
    def __init__(self, aws_access_key_id, aws_secret_access_key, is_secure=True,
            server=DEFAULT_HOST, port=None, calling_format=CallingFormat.PATH):
    ...
    def _make_request(self, method, accountid='', queue='', query_args={}, headers={}, data='', metadata={}):
        ...
        if accountid == '':
            ...
            server = "%s.%s" % (accountid, self.server)
        ...
        if (accountid != '') and (self.calling_format == CallingFormat.PATH):
            path += "/%s" % accountid
        ...
        while True:
            ...
            # add auth header
            self._add_aws_auth_header(final_headers, method, accountid, queue, query_args)
    ...
    def _add_aws_auth_header(self, headers, method, accountid, queue, query_args):
        if not headers.has_key('Date'):
            headers['Date'] = time.strftime("%a, %d %b %Y %X GMT", time.gmtime())

        c_string = canonical_string(method, accountid, queue, query_args, headers)
...
class QueryStringAuthGenerator:
    ...
    def generate_url(self, method, accountid='', queue='', query_args={}, headers={}):
        expires = 0
        ...
        canonical_str = canonical_string(method, accountid, queue, query_args, headers, expires)
        encoded_canonical = encode(self.aws_secret_access_key, canonical_str)

        url = CallingFormat.build_url_base(self.protocol, self.server, self.port, accountid, self.calling_format)

        url += "/%s" % urllib.quote_plus(queue)
